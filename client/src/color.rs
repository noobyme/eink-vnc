...
impl Color {
    pub fn gray(&self) -> u8 {
        match *self {
            Color::Gray(level) => level, //level is the stored value inside the enum variant?
    ...
    pub fn rgb(&self) -> [u8; 3] {
        match *self {
            Color::Gray(level) => [level; 3], //convert gray to rgb?
            Color::Rgb(red, green, blue) => [red, green, blue],
        }
    }

    // pub fn from_rgb(rgb: &GetPixelRgb) -> Color {
