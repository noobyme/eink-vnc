...
pub trait Framebuffer {
    ...
    // fn get_red_index(&self) -> u32 {
    //     if self.var_info.red.offset > 0 { 2 } else { 0 }
    // }

    // fn draw_gray_tile(&mut self, x: u32, y: u32, width: u32, height: u32, pixels: &[u8]) {//l,t,w,h
    //     for row in 0..height {
    //         for col in 0..width {
    //             let c = Color::Gray(pixels[(row * width + col) as usize]);
    //             self.set_pixel(x + col, y + row, c);
    //         }
    //     }
    // } so the main savings was no longer create intermediate readonlypixmap, and no getpixel just read directly
}
