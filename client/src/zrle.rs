...
impl Decoder {
        ...
        F: FnMut(Rect, Vec<u8>) -> Result<bool>,
    {
        ...
        fn copy_indexed(palette: &[u8], pixels: &mut Vec<u8>, bpp: usize, index: u8) -> Result<()> {
            let start = index as usize * bpp;
            let end = start + bpp;
            if end > palette.len() {
                return Err(Error::Unexpected("ZRLE palette index out of range"));
            }
            pixels.extend_from_slice(&palette[start..end]);
            Ok(())
        }
        ...
        let (compressed_bpp, pad_pixel) =
            if format.bits_per_pixel == 32 && format.true_colour {
        ...
        while y < rect.height {
            ...
            while x < rect.width {
                ...
                match (is_rle, palette_size) {
                    ...
                    (true, 2..=127) => {
                        // Indexed RLE: each run byte = [bit 7 = longer_than_one][bits 6..0 = palette index]
                        // per RFB spec: "The high bit indicates whether the run is longer than one pixel;
                        // the remaining bits are the palette index."
                        let mut count = 0;
                        while count < pixel_count {
